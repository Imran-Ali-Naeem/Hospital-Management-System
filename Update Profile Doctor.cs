...
namespace HMS
{
    public partial class Update_Profile_Doctor : Form
    {
        OracleConnection con;
        int userId;

        public Update_Profile_Doctor()
        {
            InitializeComponent();
        }
        ...
        public Update_Profile_Doctor(int userID)
        {
            InitializeComponent();
        ...
        private void LoadProfile(int userId)
        {
            if (userId != -1) // Check if valid user ID is found
            {
                ...
                {
                    con.Open();
                    OracleCommand getProfile = new OracleCommand("SELECT ID, USER_NAME, PASSWORD, FNAME, LNAME, GMAIL, gender, CNIC, BloodGroup, REGDATE, DOB, PHONE, SPECIALIZATION_EDU,LISCENCE_NO , BASIC_SAL, AID FROM DOCTOR WHERE ID = :parm1", con);
        ...
        {
            OracleCommand updateCmd = new OracleCommand("UPDATE DOCTOR SET PASSWORD = :PASSWORD, FNAME = :FNAME, LNAME = :LNAME, GMAIL = :GMAIL, gender = :gender, CNIC = :CNIC, BloodGroup = :BloodGroup, DOB = :DOB, PHONE = :PHONE, SPECIALIZATION_EDU = :SPECIALIZATION_EDU WHERE ID = :parm1", con);
        ...
        private void Update_Profile_Doctor_Load(object sender, EventArgs e)
        {
